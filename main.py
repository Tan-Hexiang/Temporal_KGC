...
def train_one_epoch(model, dataloader, optimizer, args, epoch, global_count, writer):
    ...
    with tqdm(dataloader, desc=f"Train Ep {epoch}", mininterval=60) as tq:
        for batch in tq:
            ...
            batch["start_time_id"] = batch["start_time_id"].to(args.device)
            batch["end_time_id"] = batch["end_time_id"].to(args.device)
            batch["start_time_list"] = batch["start_time_list"].to(args.device)
            batch["end_time_list"] = batch["end_time_list"].to(args.device)
            
            batch["graph"] = batch["graph"].to(args.device)
            batch["example_idx"]=batch["example_idx"].to(args.device)
...
def main(args):
    ...
    train_dataset, valid_dataset, test_dataset = get_datasets(filenames, args)
